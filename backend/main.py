...
5) name: 字符串，候选人姓名（从简历中提取的全名，没有则返回空字符串）
   - 注意区分姓名和标题/标语，不要将"个人简历"、"简历"等标题当作姓名
   - 如果找到多个可能的名字，取第一个

6) education: 字符串，最高学历（如："本科"、"硕士"、"博士"、"大专"、"MBA"等，没有则返回空字符串）

7) school: 字符串，毕业院校名称（如："北京大学"、"MIT"等，没有则返回空字符串）

8) major: 字符串，专业名称（如："计算机科学"、"金融学"等，没有则返回空字符串）

9) key_skills: 数组，关键技能列表，5-10个核心技能
   - 从简历中提取真实的技能，不要编造
   - 每个技能是字符串，如："Python"、"数据分析"、"项目管理"
   - 如果没有技能信息，返回空数组 []

10) work_experience_years: 整数，工作年限
    - 从简历的工作经历中推断总工作年限
    - 应届生或无工作经验返回 0
    - 实习经历算 0 年，全职工作才算年限

11) job_recommendations: 数组，推荐6个适合该候选人的岗位，覆盖不同行业，每项包含：
   - title: 岗位名称
...
12) resume_diagnosis: 对象，对简历文本进行严格的质量诊断，包含：
...
def _add_default_values_for_new_fields(parsed_data: dict[str, Any]) -> dict[str, Any]:
    ...
    # ── 个人信息字段默认值 ──
    if "name" not in parsed_data or not isinstance(parsed_data.get("name"), str):
        parsed_data["name"] = ""
    if "education" not in parsed_data or not isinstance(parsed_data.get("education"), str):
        parsed_data["education"] = ""
    if "school" not in parsed_data or not isinstance(parsed_data.get("school"), str):
        parsed_data["school"] = ""
    if "major" not in parsed_data or not isinstance(parsed_data.get("major"), str):
        parsed_data["major"] = ""
    if "key_skills" not in parsed_data or not isinstance(parsed_data.get("key_skills"), list):
        parsed_data["key_skills"] = []
    else:
        parsed_data["key_skills"] = [
            s for s in parsed_data["key_skills"]
            if isinstance(s, str) and s.strip()
        ][:10]
    if "work_experience_years" not in parsed_data or not isinstance(parsed_data.get("work_experience_years"), int):
        parsed_data["work_experience_years"] = 0
    else:
        parsed_data["work_experience_years"] = max(0, parsed_data["work_experience_years"])
...
【简历上下文处理 — 重要】
- 如果本条 system prompt 中包含了"用户已上传简历"的标记，说明用户已上传简历。
  简历中已有用户的完整信息（教育背景、技能、工作经历等）。
  必须直接使用这些已知信息，**绝对不要询问简历中已有的内容**。
  只追问简历中未体现的细节。
- 如果提示中包含"用户尚未上传简历"的标记，说明用户没有简历。
  请主动引导用户提供基本信息（专业、经历、求职方向等），或建议上传简历。
...
【简历上下文处理 — 重要】
如果上下文中包含简历信息，说明用户已上传简历。必须直接使用这些已知信息，不要询问简历中已有的内容。
如果上下文中提示用户尚未上传简历，请主动引导用户提供背景信息或上传简历。"""
...
【简历上下文处理 — 重要】
如果上下文中包含简历信息，说明用户已上传简历。直接使用这些已知信息进行优化，不要询问简历中已有的内容。
如果上下文中提示用户尚未上传简历，请主动引导用户先上传简历或提供基本信息。"""
...
【简历上下文处理 — 重要】
如果上下文中包含简历信息，说明用户已上传简历。直接使用这些已知信息进行规划，不要询问简历中已有的内容。
如果上下文中提示用户尚未上传简历，请主动引导用户提供背景信息或上传简历。"""
...
【简历上下文处理 — 重要】
如果上下文中包含简历信息，说明用户已上传简历。直接使用这些已知信息进行面试提问，不要询问简历中已有的内容（如学历、专业、工作经历等）。
如果上下文中提示用户尚未上传简历，请先询问用户的目标岗位和背景信息。"""
...
async def chat(request: ChatRequest) -> dict[str, Any]:
    try:
        ...
        # ── 简历上下文处理（核心逻辑：让 AI 充分感知已有信息，避免重复询问） ──
...
╔══════════════════════════════════════════════════════════╗
║           ⚠️  用户已上传简历 — 已知信息清单              ║
║  以下包含用户简历的全部内容。绝对不要询问这些已知信息。    ║
║  如果你问"你叫什么""你的专业是什么"等已有信息，           ║
║  用户会非常反感。直接基于以下信息深入对话。               ║
╚══════════════════════════════════════════════════════════╝

""", f"""━━━ 个人信息（已从简历提取，直接使用，不要再问） ━━━
【姓名】{ctx.candidate_name or "（未提取）"}
【候选人简介】{ctx.candidate_summary or "暂无"}
【最高学历】{ctx.education or "未提及"}
【毕业院校】{ctx.school or "未提及"}
【专业】{ctx.major or "未提及"}
【工作年限】{ctx.work_experience_years}年
【城市】{ctx.city or "未知"}
【MBTI】{ctx.inferred_mbti or "未知"} — {ctx.mbti_description or ""}
"""]

            # 核心技能
            if ctx.key_skills:
                resume_parts.append(f"【核心技能】{'、'.join(ctx.key_skills[:10])}\n")

            # 简历原文（最详细的信息来源）
            if ctx.resume_raw_text:
                resume_parts.append(f"""
【简历原文 — AI从文件中提取的完整内容（供你了解用户的教育、技能、经历等）】
```
{ctx.resume_raw_text[:2000]}
```
""")
...
""")
        ...
        elif ctx and not ctx.has_resume:
            resume_section = """

╔══════════════════════════════════════════════════════════╗
║           📌 用户尚未上传简历                           ║
║  请主动引导用户提供基本信息或上传简历文件。               ║
║  可以先问用户的专业、工作经历、求职方向等，               ║
║  也可以建议用户上传 PDF/DOCX 简历。                     ║
╚══════════════════════════════════════════════════════════╝
